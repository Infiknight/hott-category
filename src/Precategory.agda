...
module Precategory where
  ...
  record Precategory : Set₁ where
    field
      ...
      id-arrow : (x : objects) → arrows x x

      homs-are-hsets : (x y : objects) → is-hset(arrows x y)

      _∘_ : {x y z : objects} → (arrows y z) → (arrows x y)  → (arrows x z)
      ∘-unit-l : {x y : objects} {f : arrows x y} → ((id-arrow y) ∘ f) == f
      ∘-unit-r : {x y : objects} {f : arrows x y} →  (f ∘ id-arrow x) == f
      assoc : {x y z w : objects} {f : arrows x y} {g : arrows y z} {h : arrows z w} → (h ∘ (g ∘ f)) == ((h ∘ g) ∘ f)
  ...
  module _ where
    ...
    _::_∘_ : (C : Precategory) → {x y z : objects C} → (arrows C y z) → (arrows C x y) → (arrows C x z)
